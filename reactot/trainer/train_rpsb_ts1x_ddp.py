...
os.environ['NCCL_SOCKET_IFNAME'] = 'eth0'  # 或 'lo' 本地回环
...
training_config = dict(
    ...
    sampler_config=dict(
        ...
        # max_num=5600,  # This is for 32GB GPU; Scale linearly with memory
        mode="node^2",
        shuffle=True,
        ddp=True,
    )
...
# 当前代码中已经设置了strategy，但需要确保正确配置
devices = list(range(torch.cuda.device_count()))  # 使用所有可用的GPU
strategy = DDPStrategy(find_unused_parameters=True)
...
trainer = Trainer(
    ...
    # replace_sampler_ddp=False,  # 这一行在PyTorch Lightning 2.x中已移除，需要删除
)
...
python -m reactot.trainer.train_rpsb_ts1x_ddp | tee oa_mix_train_dim_ddp.log
